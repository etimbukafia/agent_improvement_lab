...
from agent_improvement_lab.contracts.cases import NumericRange, ToolCallExpectation
from agent_improvement_lab.contracts.failures import FailureCategory
from agent_improvement_lab.contracts.traces import ToolCallOutcome
...
def _expected_calls(context: EvaluationContext) -> list[ToolCallExpectation]:
    return sorted(context.case.tool_expectations, key=lambda expectation: expectation.order)


def _aligned_calls(context: EvaluationContext) -> list[tuple[ToolCallExpectation, Any | None]]:
    ...
    def evaluate(self, context: EvaluationContext) -> EvaluationOutcome:
        ...
        for expectation, call in _aligned_calls(context):
            if call is None or call.name != expectation.name:
                continue
            if any(key not in call.arguments for key in expectation.required_arguments):
                continue
            if any(
                call.arguments.get(key) != value
                for key, value in expectation.exact_arguments.items()
            ):
                continue
            matched += 1
        denominator = max(len(expected), len(actual), 1)
...
def _constraint_matches(expectation: ToolCallExpectation, arguments: dict[str, Any]) -> bool:
    if any(key not in arguments for key in expectation.required_arguments):
        return False
    if any(arguments.get(key) != value for key, value in expectation.exact_arguments.items()):
        return False
    if any(
        not _matches_type(arguments.get(key), type_name)
        for key, type_name in expectation.argument_types.items()
    ):
        return False
    if any(arguments.get(key) not in values for key, values in expectation.allowed_values.items()):
        return False
    if any(
        re.fullmatch(pattern, str(arguments.get(key, ""))) is None
        for key, pattern in expectation.patterns.items()
    ):
        return False
    if any(
        not _matches_range(arguments.get(key), bounds)
        for key, bounds in expectation.numeric_ranges.items()
    ):
        return False
    return True
...
class ProtectedArgumentIntegrity(LabEvaluator):
    ...
    def evaluate(self, context: EvaluationContext) -> EvaluationOutcome:
        ...
        for expectation in _expected_calls(context):
            for key in expectation.protected_arguments:
                protected.append((expectation.name, key, expectation.exact_arguments.get(key)))
        if not protected:
