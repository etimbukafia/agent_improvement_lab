...
from agent_improvement_lab.contracts.cases import DatasetVersion, EvaluationCaseRef, RiskLevel
...
def infer_failure_category(evaluator_id: str) -> FailureCategory:
    ...
    if "safety" in normalized or "authorization" in normalized or "verification" in normalized:
...
def cluster_failures(
    ...
    *,
    ...
    groups: defaultdict[tuple[str, str, str, tuple[str, ...], str], list[EvaluationFailure]] = (
        defaultdict(list)
    )
...
def _expected_behavior(case: EvaluationCaseRef) -> str:
    declared = case.metadata.get("expected_behavior")
    if isinstance(declared, str) and declared.strip():
        return declared
    expected_declared = case.expected.get("expected_behavior")
    if isinstance(expected_declared, str) and expected_declared.strip():
        return expected_declared
    if case.expected:
        return stable_json_dumps(case.expected)
    return "The case should satisfy its declared evaluation criteria."


def _intent(case: EvaluationCaseRef) -> str | None:
...
def _severity(risk: RiskLevel, category: FailureCategory) -> Severity:
    if category == FailureCategory.SAFETY:
