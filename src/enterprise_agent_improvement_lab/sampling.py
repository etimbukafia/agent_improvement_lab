...
from enterprise_agent_improvement_lab.contracts.traces import ExecutionTrace, ToolCallOutcome
...
def _has_tool_error(traces: Sequence[ExecutionTrace], scores: Sequence[EvaluationScore]) -> bool:
    return any(
...
def _has_repeated_clarification(
    scores: Sequence[EvaluationScore], traces: Sequence[ExecutionTrace]
) -> bool:
    if any(
    ...
    for trace in traces:
        questions = [
            normalize_text(message.message_summary or "")
            for message in ordered_messages(trace)
            if message.role.casefold() in {"user", "human"} and message.message_summary
        ]
